...
exports.coerceLocationGraph = coerceLocationGraph;
exports.validateLocationGraph = validateLocationGraph;
exports.initLocationState = initLocationState;
exports.coerceLocationState = coerceLocationState;
exports.evalUnlockConditions = evalUnlockConditions;
exports.evaluateUnlocks = evaluateUnlocks;
exports.applyDiscovers = applyDiscovers;
exports.evaluateNpcPlacements = evaluateNpcPlacements;
exports.evaluateEncounters = evaluateEncounters;
exports.positionOf = positionOf;
exports.applyActorMove = applyActorMove;
exports.eligibleCombatTargets = eligibleCombatTargets;
exports.entryNodeOf = entryNodeOf;
exports.computeExits = computeExits;
exports.findPathBlocker = findPathBlocker;
exports.resolveTravelIntent = resolveTravelIntent;
exports.resolveMoveTarget = resolveMoveTarget;
exports.looksLikeTravel = looksLikeTravel;
exports.matchEvidence = matchEvidence;
exports.buildLocationBlock = buildLocationBlock;
exports.locationShortName = locationShortName;
exports.nodesFromLegacyLocations = nodesFromLegacyLocations;
const npc_1 = require("@/lib/game/npc");
// ── Coercion / validation ─────────────────────────────────────────────────────
const STATUSES = ["hidden", "discovered", "unlocked"];
function asStr(v) {
...
function coerceEvidence(v) {
    if (!Array.isArray(v))
        return [];
    return v
        .filter((e) => e && typeof e === "object" && asStr(e.id))
        .map((e) => ({
        id: asStr(e.id),
...
function coerceNpcPlacements(v) {
    if (!Array.isArray(v))
        return [];
    return v
        .filter((p) => p && typeof p === "object" && asStr(p.npc) && asStr(p.at))
        .map((p) => ({
        npc: asStr(p.npc),
        at: asStr(p.at),
...
function coerceNpcEncounters(v) {
    if (!Array.isArray(v))
        return [];
    return v
        .filter((e) => e && typeof e === "object" && asStr(e.npc) && Array.isArray(e.when) && e.when.length > 0)
        .map((e) => ({
        npc: asStr(e.npc),
...
function coerceEdges(v) {
    if (!Array.isArray(v))
        return [];
    const out = v
        .filter((e) => e && typeof e === "object" && asStr(e.from) && asStr(e.to))
        .map((e) => ({
        from: asStr(e.from),
        to: asStr(e.to),
...
function coerceLocationGraph(raw) {
    const nodesRaw = Array.isArray(raw?.nodes) ? raw.nodes : Array.isArray(raw) ? raw : null;
    if (!nodesRaw)
        return null;
    const nodes = nodesRaw
        .filter((n) => n && typeof n === "object" && asStr(n.id) && asStr(n.name))
        .map((n) => ({
        id: asStr(n.id),
        name: asStr(n.name),
        desc: asStr(n.desc),
    ...
    if (!deduped.some((n) => n.initial === "unlocked"))
        deduped[0].initial = "unlocked";
    ...
    for (const n of deduped) {
        if (n.container && !containerIds.has(n.container))
...
function validateLocationGraph(graph, npcRefs, objectiveIds) {
    const warnings = [];
    ...
    function validateTerms(terms, context) {
        for (const group of terms) {
            for (const term of group) {
                ...
                if (kind === "visit" || kind === "after") {
                    if (!ids.has(parts[1] ?? ""))
                        warnings.push(`${context} 引用了不存在的地點 id：${term}`);
                }
                else if (kind === "item") {
                    if (!evidenceIds.has(parts[1] ?? ""))
                        warnings.push(`${context} 引用了不存在的證物 id：${term}`);
                }
                else if (kind === "count") {
                    if (!evidenceTags.has(parts[1] ?? ""))
                        warnings.push(`${context} 引用了沒有任何證物使用的標籤：${term}`);
                    if (!Number.isFinite(Number(parts[2])))
                        warnings.push(`${context} 的 count 條件缺少數量：${term}`);
                }
                else if (kind === "round") {
                    if (!Number.isFinite(Number(parts[1])))
                        warnings.push(`${context} 的 round 條件缺少回合數：${term}`);
                }
                else if (kind === "objective") {
                    if (objectiveIds && !objectiveIds.has(parts[1] ?? ""))
                        warnings.push(`${context} 引用了不存在的任務目標：${term}`);
                }
                else {
    ...
    for (const n of graph.nodes) {
        for (const ref of n.discovers) {
            if (!ids.has(ref))
                warnings.push(`地點「${n.name}」的 discovers 引用了不存在的地點 id：${ref}`);
        }
    ...
    if (graph.travel_mode === "edges") {
        ...
        const resolveEndpoint = (id) => ids.has(id) ? id : entryOf(id);
    ...
    for (const p of graph.npc_placements) {
        if (!ids.has(p.at))
            warnings.push(`NPC「${p.npc}」的位置設定引用了不存在的地點 id：${p.at}`);
        if (npcRefs && p.npc && !npcRefs.has(p.npc))
            warnings.push(`NPC 位置設定中的「${p.npc}」不在此劇本的 NPC 名單中。`);
        validateTerms(p.when, `NPC「${p.npc}」的位置條件`);
    }
    for (let i = 0; i < graph.npc_encounters.length; i++) {
        const e = graph.npc_encounters[i];
        if (npcRefs && e.npc && !npcRefs.has(e.npc))
            warnings.push(`NPC 觸發事件中的「${e.npc}」不在此劇本的 NPC 名單中。`);
        validateTerms(e.when, `NPC「${e.npc}」觸發事件 ${i + 1} 的條件`);
    }
    return warnings;
}
// ── State ─────────────────────────────────────────────────────────────────────
function initLocationState(graph) {
    const status = {};
    for (const n of graph.nodes)
        status[n.id] = n.initial;
...
function coerceLocationState(raw, graph) {
    ...
    if (raw.positions && typeof raw.positions === "object") {
        for (const [cid, nid] of Object.entries(raw.positions)) {
            if (typeof nid === "string" && status[nid] !== undefined)
                positions[cid] = nid;
        }
    }
    return {
        ...
        entered_round: raw.entered_round && typeof raw.entered_round === "object"
            ? Object.fromEntries(Object.entries(raw.entered_round).filter(([k, v]) => status[k] !== undefined && Number.isFinite(Number(v))).map(([k, v]) => [k, Number(v)]))
            : base.entered_round,
...
function evalTerm(term, state, graph, currentRound, objectiveProgress) {
    const parts = term.split(":");
    switch (parts[0]) {
        case "visit":
...
function condSatisfied(when, state, graph, currentRound, objectiveProgress) {
    if (when.length === 0)
        return true;
 ...
 */
function evalUnlockConditions(when, state, graph, currentRound, objectiveProgress = {}) {
    return condSatisfied(when, state ?? EMPTY_LOCATION_STATE, graph ?? EMPTY_LOCATION_GRAPH, currentRound, objectiveProgress);
}
function unlockSatisfied(node, state, graph, currentRound, objectiveProgress) {
    if (node.unlock.length === 0)
        return false; // no conditions → only discovers[]/initial can open it
...
function evaluateUnlocks(graph, state, currentRound, objectiveProgress = {}) {
    const changes = { unlocked: [], discovered: [] };
    for (const node of graph.nodes) {
        if (state.status[node.id] === "unlocked")
            continue;
...
function applyDiscovers(graph, state, enteredNodeId) {
 ...
 * NPC is present.
 */
function evaluateNpcPlacements(graph, state, currentRound, objectiveProgress = {}, atNode) {
    const scene = atNode ?? state.current;
    if (!scene || graph.npc_placements.length === 0)
        return [];
    ...
    byNpc.forEach((placements, npc) => {
        ...
        for (const p of placements) {
            if (condSatisfied(p.when, state, graph, currentRound, objectiveProgress))
                lastSatisfied = p;
        }
        if (lastSatisfied && lastSatisfied.at === scene)
            present.push(npc);
    });
    return present;
}
 ...
 */
function evaluateEncounters(graph, state, currentRound, objectiveProgress = {}) {
    if (graph.npc_encounters.length === 0)
        return [];
    const fired = [];
    for (let i = 0; i < graph.npc_encounters.length; i++) {
        const enc = graph.npc_encounters[i];
        const key = `enc:${i}`;
        if (state.encounters_fired.includes(key))
            continue;
        // encounters need an explicit when (they can't fire "always" — that would
        // be every turn)
        if (enc.when.length === 0)
            continue;
...
function positionOf(state, characterId, graph) {
    const pos = state.positions?.[characterId];
    if (pos && state.status[pos] !== undefined)
        return pos;
    if (state.current && state.status[state.current] !== undefined)
...
function applyActorMove(graph, state, characterId, nodeId, currentRound) {
    if (!state.positions)
        state.positions = {};
...
function eligibleCombatTargets(characters, state, npcNode, graph) {
...
function entryNodeOf(graph, containerId) {
...
function buildAdjacency(graph) {
    ...
    const resolve = (id) => nodeIds.has(id) ? id : entryNodeOf(graph, id)?.id ?? null;
    for (const e of graph.edges) {
...
function openReachable(graph, adj, fromId) {
    const reached = new Set([fromId]);
    const queue = [fromId];
    while (queue.length) {
        const cur = queue.shift();
        for (const nxt of Array.from(adj.get(cur) ?? [])) {
...
function computeExits(graph, state, fromNode) {
    ...
    const queue = [origin];
    const frontierLocked = new Set();
    while (queue.length) {
        const cur = queue.shift();
        for (const nxt of Array.from(adj.get(cur) ?? [])) {
            if (reached.has(nxt) || frontierLocked.has(nxt))
                continue;
    ...
    if (curContainer?.show_locked_children) {
        for (const n of graph.nodes) {
            if (n.container === curContainer.id && state.status[n.id] !== "unlocked")
                lockedIds.add(n.id);
        }
    }
...
function findPathBlocker(graph, state, targetId, fromNode) {
    const origin = fromNode ?? state.current;
    if (!origin)
        return { hasPath: false, blocker: null };
    const adj = buildAdjacency(graph);
    ...
    const path = [];
    for (let cur = targetId; cur && cur !== origin; cur = parent.get(cur))
        path.unshift(cur);
...
function mentionScore(actionLower, name) {
    const sn = shortName(name);
    if (!sn)
        return 0;
    if (actionLower.includes(sn.toLowerCase()))
        return sn.length + 100;
    let best = 0;
    for (const seg of nameSegments(sn)) {
        if (actionLower.includes(seg.toLowerCase()) && seg.length > best)
            best = seg.length;
    }
    return best;
}
...
function matchLocationName(actionText, candidates) {
    ...
    for (const cand of candidates) {
        ...
        if (score > 0)
            scored.push({ node: cand.node, score, pos });
    }
    if (scored.length === 0)
        return null;
 ...
 */
function resolveTravelIntent(actionText, graph, state, actorNode) {
    ...
    for (const c of graph.containers) {
        ...
        if (!entry || entry.id === origin)
            continue;
        if (edgesMode && state.status[entry.id] === "hidden")
    ...
    if (!edgesMode) {
        ...
        if (status === "discovered")
            return { kind: "locked", node };
        return { kind: "unknown", node };
    }
    if (status !== "unlocked")
        return { kind: "locked", node };
    if (exits.open.some((n) => n.id === node.id))
        return { kind: "go", node };
 ...
 */
function resolveMoveTarget(name, graph, state, actorNode) {
    ...
    const pool = computeExits(graph, state, actorNode ?? state.current).open.map((n) => ({ name: n.name, node: n }));
    for (const c of graph.containers) {
        const entry = entryNodeOf(graph, c.id);
        if (entry && pool.some((p) => p.node.id === entry.id))
            pool.push({ name: c.name, node: entry });
    }
    let best = null;
    let bestScore = 0;
    for (const cand of pool) {
        ...
        if (q === full || q === sn)
            score = 1000;
        else if (full.includes(q) || q.includes(sn))
            score = 100 + Math.min(q.length, full.length);
        else
            score = mentionScore(q, cand.name);
        if (score > bestScore) {
            best = cand.node;
            bestScore = score;
        }
...
function looksLikeTravel(actionText) {
...
function howIsGenericSearch(how) {
    const h = (how ?? "").trim();
 ...
 */
function matchEvidence(actionText, graph, state, isSearch, atNode) {
    const scene = atNode ?? state.current;
    if (!scene)
        return [];
    ...
    if (isSearch) {
        for (const e of unfound) {
            if (!taken.has(e.id) && howIsGenericSearch(e.how))
                out.push(e);
        }
    }
    return out;
...
function buildLocationBlock(graph, state, travel, stuckHint, currentRound, firedEncounters = [], objectiveProgress = {}, npcRoster = [], scene) {
    ...
    const lines = ["LOCATION SYSTEM (server-authoritative — you MUST follow this; you cannot reveal hidden places yourself, and a character's position only changes via the move_to field below or a TRAVEL notice from the system):"];
    if (current) {
        ...
        if (unfound.length) {
            lines.push(`EVIDENCE OBTAINABLE HERE (lock behind successful checks; do NOT volunteer): ${unfound
    ...
    if (scene) {
        ...
        if (scene.sceneFacts && scene.sceneFacts.length) {
            lines.push(`SCENE HISTORY (things that already happened AT THIS LOCATION — keep physical continuity with them; do not reset, undo, or re-describe them as new): ${scene.sceneFacts.join("；")}`);
        }
        // Where every character stands. Characters elsewhere are NOT in this scene.
        lines.push(`PARTY WHEREABOUTS: ${scene.whereabouts.map((w) => `${w.name} @ ${nodeName(w.node)}`).join(" · ")}. ` +
    ...
    if (graph.npc_placements.length > 0) {
        if (npcsHere.length > 0) {
            lines.push(`NPCS PRESENT HERE: ${npcsHere.map((ref) => (0, npc_1.npcDisplayName)(ref, npcRoster)).join("、")}`);
        }
    ...
    if (found.length)
        lines.push(`EVIDENCE THE PARTY HOLDS: ${found.map((e) => e.name).join("、")}`);
    ...
    if (unlockedOthers.length)
        exitsParts.push(`可前往：${unlockedOthers.map((n) => shortName(n.name)).join("、")}`);
    if (discoveredLocked.length)
        exitsParts.push(`已知但尚未能進入：${discoveredLocked.map((n) => shortName(n.name)).join("、")}`);
    if (exitsParts.length)
        lines.push(`KNOWN LOCATIONS — ${exitsParts.join(" | ")}`);
    ...
    lines.push(`MOVING: if the acting player's action means going to one of the 可前往 locations (however they phrase it — partial name, "回去那裡", a typo), narrate ${mover} moving there and set "move_to" in your JSON to that location's EXACT name from the list. If they try somewhere locked or unknown, ${mover} STAYS at the scene's location — narrate why entry fails and set move_to to null. If the system already announced "TRAVEL THIS TURN" below, the move is done: set move_to to null. Never narrate ${mover} being anywhere except this scene's location or a move_to/TRAVEL destination.${scene ? ` move_to moves ONLY ${scene.actorName} — other characters stay where they are.` : ""}`);
    if (travel) {
        if (travel.kind === "arrived") {
            lines.push(`TRAVEL THIS TURN: ${mover} has MOVED to ${travel.node.name}. Narrate the transition and the new scene.${travel.firstVisit && travel.node.on_enter ? ` FIRST-VISIT BEAT: ${travel.node.on_enter}` : ""}`);
        }
        else if (travel.kind === "soft_wall") {
            lines.push(`TRAVEL BLOCKED: ${mover} tried to go to ${travel.node.name}, which is NOT yet accessible. Narrate an in-world reason entry fails${travel.node.locked_narration ? `（建議：${travel.node.locked_narration}）` : ""}. You may hint at what might open the way, but do NOT let them in. ${mover} stays where they are.`);
        }
        else if (travel.kind === "blocked_path") {
            lines.push(travel.blocker
                ? `TRAVEL BLOCKED EN ROUTE: ${mover} tried to reach ${travel.node.name}, but the way there passes through ${travel.blocker.name}, which is not yet passable${travel.blocker.locked_narration ? `（建議：${travel.blocker.locked_narration}）` : ""}. Narrate that the route is cut off at ${travel.blocker.name} — do NOT teleport them past it. ${mover} stays where they are; set move_to to null.`
            ...
            lines.push(`UNKNOWN PLACE: the actor referred to a place the party has not learned about. Treat it as in-character speculation — the world gives no confirmation it exists.`);
        }
    ...
    if (scene && scene.nextNode) {
        ...
        const nextParts = [];
        if (nextExits.open.length)
            nextParts.push(`可前往：${nextExits.open.map((n) => shortName(n.name)).join("、")}`);
        if (nextExits.locked.length)
            nextParts.push(`看得到但進不去：${nextExits.locked.map((n) => shortName(n.name)).join("、")}`);
        lines.push(`NEXT TURN'S SCENE (for the 3 suggested choices ONLY): ${scene.nextName} is at ${shortName(nextNodeDef?.name ?? scene.nextNode)}. ` +
    ...
    for (const enc of firedEncounters) {
        lines.push(`NPC ENCOUNTER THIS TURN — ${(0, npc_1.npcDisplayName)(enc.npc, npcRoster)} arrives / makes contact with the party regardless of location. Weave this into the scene immediately. Beat: ${enc.beat}`);
    }
    if (stuckHint) {
 ...
 * before.
 */
function nodesFromLegacyLocations(locs) {
    return locs
        ...
        const descParts = [];
        if (l.clues?.trim())
            descParts.push(`線索：${l.clues.trim()}`);
        if (l.items?.trim())
            descParts.push(`物品：${l.items.trim()}`);
