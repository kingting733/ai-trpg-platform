...
/** True if the parsed object is already a scenario JSON (e.g. generated by an
 *  external AI following our format spec) rather than a prose story document. */
function looksLikeScenarioJSON(parsed: any): boolean {
...
export async function analyzeScenarioDocument(
  ...
  if (trimmed.startsWith("{") || /^```(?:json)?\s*\{/i.test(trimmed)) {
    try {
      ...
      // Not valid JSON after all — fall through to the AI analysis below.
    }
  }
