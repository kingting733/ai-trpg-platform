...
4b. 用劇本裡的正式名字（「王伯把備用鑰匙交給隊伍」，不是「說服那個管理員」）；裁判靠名字對上敘事。
4c. 系統本來就知道的事不要寫成目標：拿到證物用 item:、到過地點用 visit:、NPC 死活用 npc_dead:/npc_alive:、撐到第 N 回合用 round:，這些直接寫進結局或解鎖條件。目標只留「只有 AI 讀得懂的事」——NPC 答應了什麼、儀式做完了、某句話被說出口。
4d. 每一條目標都要被某個結局、解鎖、NPC 位置、觸發或情報引用；沒有人引用的目標是純成本，刪掉。必要目標 2–4 條；「活著離開」這類持續狀態用結局條件，失敗情境設 failure 結局，給玩家看的說明寫在簡介（目標文字玩家看不到）。完整規則見 docs/creator-guide/objectives.md。
